...
Проведено 11 реальных API-запросов. Официальный запуск воспроизводит первый
валидный ответ из `artifacts/llm_policy.json` без сети; при изменении входных
данных переходит к численной политике. Для обновления решения на новых данных
есть live-режим с ключом в `.env`. Веса модели пока не обучались.
...
Прошли **147 офлайн-тестов** и 10 live-прогонов с LLM. Модель во всех случаях
выбрала гипотезы, уже входившие в базовый план: результат совпал с baseline.
Улучшение прибыли от LLM на этом наборе не подтверждено. Default replay
воспроизводит тот же результат. [Полный отчёт LLM](reports/LLM_RELEASE.md).
Адаптивный размер повторных пилотов реализован отдельной опцией; в тесте он
снизил минимум с 1,568 млн до 1,305 млн, поэтому по умолчанию выключен.
...
Максимальное измеренное время полного adaptive-evaluation: **0.599 с**
